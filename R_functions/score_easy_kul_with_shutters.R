score_easy_kul_with_shutters <-
            ...
            plotting = TRUE, electro = FALSE, pref = 4)
  {
   ...
   ### this part extracts all the models for each single plant in my plants
    ...
    if(plotting == TRUE){
      #### PLOTTING PART ###
      pdf(file = file.path("C:/Users/kshresth/back up C drive_ 29.8.2018 KSHRESTH/MeioMSI/Results/electrograms with shutters_pdfs",paste (Plot_name,cfp[cols],".pdf")), paper="a4",height= 12, width =8)
      par(mfrow=c(4,2))
      ...
      for(g in 1:length(n.inds)){
        ...
        plot( new.whole.data[[g]]$xx, new.whole.data[[g]]$yy, type="l", col=cfp[cols], xaxt="n",
              xlim=c(xm,xl),  ylim=c(0,mylim +100), ylab="Intensity",  # real code ma 0 ko sato ylim=c(lower.bounds[g],mylim +100)
        ...
        text(x=list.weis[[g]]$wei- 0.5, y= mylim-50, srt= 90, labels=round(list.weis[[g]]$wei, digits = 2),cex=0.75)
